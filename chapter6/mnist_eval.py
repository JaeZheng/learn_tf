...
from chapter6 import mnist_inference, mnist_train
...
def evaluate(mnist):
    ...
    reshaped_x = np.reshape(mnist.validation.images, (mnist.validation.num_examples,
                                                      mnist_inference.image_size,
                                                      mnist_inference.image_size,
                                                      mnist_inference.num_of_channels))
    validate_feed = {x: reshaped_x, y_true: mnist.validation.labels}
    ...
    variable_averages = tf.train.ExponentialMovingAverage(mnist_train.moving_average_rate)
